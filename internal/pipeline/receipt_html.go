...
<p>Review policy: the initial pass reviews the complete base-to-head diff and freezes its manifest. Convergence revalidates those frozen findings; only a new P0/P1 proven in the prior-head-to-current-head correction delta can join the blocking ledger. New P2/P3 findings and unrelated pre-existing issues do not start another correction loop. Initial and blocking regression findings must identify a current added or modified line in the applicable diff, or line 0 only for deletion-only, deleted, or pure-rename file-level evidence.</p>
