	...
	if cfg.CI.Managed {
		for _, provider := range cfg.CI.Providers {
			if len(cfg.CI.SecretBindings[provider]) > 0 {
				...
				if provider == "gitlab" {
					requireFile(&report, root, ".sam-harness/WORKFLOW.md", cfg.CI.AgentControlPlanes[provider].ExternalProject, "external trusted control plane", "absence blocks merge")
				}
			}
			switch provider {
			case "github":
			...
			case "gitlab":
				...
				if cfg.Workflow != nil && cfg.Workflow.Enabled {
					...
					if len(scopedSecretBindings(cfg, "gitlab", model.CISecretScopeReview)) == 0 {
						fragments = append(fragments, "sam-harness-review:")
					}
					if cfg.Workflow.Correction.OpenChangeRequest && len(scopedSecretBindings(cfg, "gitlab", model.CISecretScopeRepair)) == 0 {
...
func validateGitLabWorkflow(report *Report, root, path string, cfg model.Config) {
	...
	reviewBound := len(scopedSecretBindings(cfg, "gitlab", model.CISecretScopeReview)) > 0
	repairBound := len(scopedSecretBindings(cfg, "gitlab", model.CISecretScopeRepair)) > 0
	localRepair := repairEnabled(cfg) && !repairBound
	...
	if reviewBound {
		...
		if err == nil {
			content := string(data)
			forbidFragments(report, path, content, "sam-harness-review:\n", cfg.CI.AgentSecretEnvironments["gitlab"])
			if repairBound {
