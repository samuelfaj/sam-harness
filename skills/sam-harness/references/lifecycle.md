...
The review phase is a pre-merge gate over the exact proposed base-to-head change. Give `--review-base` a separate absolute base-checkout directory. `--review-base-sha` and `--review-head-sha` are a paired 40- or 64-character hexadecimal identity; secret-bearing review requires all three flags. They are valid only for `review` or `all`. Runtime requires each SHA to equal its checkout's Git `HEAD`, snapshots both sides, creates a canonical patch, and records `review_base_root`, `review_base_sha`, `review_base_fingerprint`, `review_head_sha`, `review_head_fingerprint`, `review_patch`, and `review_patch_sha256`. Every initial finding must identify a current added or modified line in the base-to-head patch, or line 0 only for deletion-only, deleted, or pure-rename file-level evidence. An out-of-scope P0/P1 is malformed and blocks; an out-of-scope P2/P3 is excluded from correction and recorded separately for human inspection. Missing proof fails closed. It rechecks the Git identities and contents after all reviewers; a changed input blocks. A local head-only review without `--review-base` does not satisfy the pre-merge delta gate. A convergence re-review may pass `--prior-review-receipt` to bind one frozen manifest: same-role frozen IDs remain eligible, resolved IDs are recorded, and only new P0/P1 findings satisfying the same prior-head-to-current-head scope rule can block; unrelated pre-existing findings do not start another loop.
