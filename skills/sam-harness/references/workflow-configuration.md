...
GitHub mixed bindings move only the bound scope. Credential-free review remains in the ordinary GitHub workflow when only repair is bound, and credential-free static/test/artifact repair remains there when only review is bound. GitLab `mode: external` is authoritative regardless of binding mix: the external project owns all agent review, correction, and publishing, and the MR pipeline emits none of those jobs. No-secret correction commands may remain repository-relative only where the provider does not delegate the whole agent control plane externally.
...
The required review is pre-merge and must evaluate the exact proposed SHA and diff. Every initial finding must identify a current added or modified line in a base-to-head hunk, or line 0 only for deletion-only, deleted, or pure-rename file-level evidence. Out-of-scope P0/P1 findings are malformed and block; out-of-scope P2/P3 suggestions are excluded from the correction manifest and recorded separately in the HTML receipt. A generated workflow may repeat review on the trusted default branch, but that later run never substitutes for the pre-merge gate. Secret-bearing review uses `--config` for `.sam-harness/config.yaml` in a separate trusted base checkout plus `--review-base`, `--review-base-sha`, and `--review-head-sha`. Runtime compares the 40- or 64-character provider SHAs with both Git `HEAD` values before and after the reviewers. The receipt binds those SHAs, both fingerprints, the canonical patch, its SHA-256, and a lineage digest. A repair-branch re-review may pass `--prior-review-receipt`: frozen unresolved actions remain eligible only through a same-role manifest ID, while new findings follow the same scope rule in prior-head..current-head; pre-existing findings do not start another loop and missing proof fails closed. Review and repair receipts have escaped standalone HTML companions.
...
Repeat the object for `security`, `correctness`, `test_quality`, `business_rules`, and `simplicity`, substituting the actual approved argv. Review commands receive the role in `SAM_HARNESS_REVIEW_ROLE` and the structured prompt on standard input. Their exact JSON must match `.sam-harness/reviewer-output.schema.json`: `review_complete` is true, each finding includes its exact `required_change` and observable `acceptance`, and the optional finding `id` is used only to refer to a frozen same-role action during convergence. Every reviewer reports all actionable findings in its role in the same pass and reviews only the altered diff under the scope rule above. `filesystem_read_only: true` is a required user attestation that the chosen executable is independently configured for read-only filesystem access.
