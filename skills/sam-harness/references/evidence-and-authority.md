...
GitLab's generated MR YAML omits every agent review, correction, and publisher job when `mode: external`. Its external trusted project owns that complete agent control plane and must publish the configured status against the exact current MR head using protected variables/environment and provider-side approvals, regardless of binding mix. Read those controls back; Sam Harness does not supply that loop inside the target repository. Missing App/external status, protected credentials, released runtime, trusted base configuration, or unchanged current head blocks instead of silently skipping.
