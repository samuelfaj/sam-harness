...
- Every initial review finding must identify a current added or modified line in the base-to-head diff, or line 0 only for deletion-only, deleted, or pure-rename file-level evidence; other out-of-diff paths and lines are rejected. A re-review may take an optional `--prior-review-receipt`: frozen unresolved actions remain eligible by their same-role manifest ID, while new findings follow the same scope rule in prior-head..current-head and new P0/P1 findings alone can block. Missing proof fails closed. Each JSON receipt has an escaped standalone HTML companion beside it.
