...
- Review convergence is fixed: review the complete base-to-head diff once, freeze that manifest, revalidate its findings by same-role ID, and allow only a new P0/P1 proven in prior-head..current-head to join the blocking ledger. New P2/P3 findings and unrelated pre-existing issues do not start another correction loop. Every initial or blocking regression finding must identify a current added or modified line in its applicable diff, or line 0 only for deletion-only, deleted, or pure-rename file-level evidence. Missing proof fails closed. Each JSON receipt has an escaped standalone HTML companion beside it.
