...
- Every initial review finding must identify a current added or modified line in the base-to-head diff, or line 0 only for deletion-only, deleted, or pure-rename file-level evidence. Out-of-scope P0/P1 findings are rejected; out-of-scope P2/P3 suggestions are excluded from correction and recorded separately in the HTML receipt. A re-review may take an optional `--prior-review-receipt`: frozen unresolved actions remain eligible by their same-role manifest ID, while new findings follow the same scope rule in prior-head..current-head and new P0/P1 findings alone can block. Missing proof fails closed. Each JSON receipt has an escaped standalone HTML companion beside it.
