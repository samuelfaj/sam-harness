...
```bash
sam-harness upgrade /ruta/al/repositorio --to 0.5.0 --answers /tmp/sam-harness-respuestas-v0.5.json
```
...
- Los seis revisores son `architecture`, `security`, `correctness`, `test_quality`, `business_rules` y `simplicity`. Sus comandos deben configurarse y atestiguarse de forma independiente como de solo lectura en el filesystem. El prompt y el recibo pre-merge vinculan el fingerprint de la base, el fingerprint del head, el parche canónico y su SHA-256. JSON inválido, un hallazgo P0/P1, una mutación del repositorio o un cambio de base/head bloquean la revisión; los hallazgos P2/P3 permanecen registrados.
