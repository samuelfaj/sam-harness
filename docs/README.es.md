...
```bash
sam-harness upgrade /ruta/al/repositorio --to 0.6.0 --answers /tmp/sam-harness-respuestas-v0.6.json
```
...
- Los seis revisores son `architecture`, `security`, `correctness`, `test_quality`, `business_rules` y `simplicity`. Sus comandos deben configurarse y atestiguarse de forma independiente como de solo lectura en el filesystem. Cada uno devuelve `review_complete: true` y todo hallazgo accionable con `required_change` exacto y `acceptance` observable. El prompt y el recibo pre-merge vinculan el fingerprint de la base, el fingerprint del head, el parche canónico, su SHA-256 y un manifiesto de reparación con hash que contiene todos los hallazgos. JSON inválido o incompleto, un hallazgo P0/P1, una mutación del repositorio o un cambio de base/head bloquean la revisión; los hallazgos P2/P3 permanecen en el mismo manifiesto.
