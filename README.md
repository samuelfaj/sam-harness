...
`--review-base`, `--review-base-sha`, and `--review-head-sha` are valid only for `review` or `all`; the SHA flags must appear together and require the base directory. Secret-bearing review requires all three. The 40- or 64-character hexadecimal SHAs must equal the checked-out base and target `HEAD` before and after review. The receipt records `review_base_root`, `review_base_sha`, `review_base_fingerprint`, `review_head_sha`, `review_head_fingerprint`, the canonical `review_patch` artifact, and `review_patch_sha256`; reviewer prompts carry only `review_patch_path` and `review_patch_sha256`, so reviewers read the untrusted diff from the isolated sandbox without embedding its bytes. Any identity or content drift blocks. A head-only local review without `--review-base` may still be useful, but it does not satisfy the required pre-merge delta gate.

Review convergence has one fixed policy: the initial pass reviews the complete base-to-head MR diff and freezes its manifest; later passes revalidate those frozen findings by their same-role IDs; only a new P0/P1 proven on the prior-head-to-current-head correction delta may join the blocking ledger. A complete passed initial review with non-blocking findings is a valid prior receipt, so P2/P3 corrections do not trigger another full review. New P2/P3 findings and unrelated pre-existing issues do not start another correction loop. Initial findings and new P0/P1 regressions must identify a current added or modified line in their applicable patch, or line `0` only for deletion-only, deleted, or pure-rename file-level evidence. An out-of-scope P0/P1 remains malformed and blocks; an out-of-scope P2/P3 is excluded from correction and recorded separately in the HTML receipt for human inspection. Missing proof fails closed. Every JSON receipt is accompanied by an escaped standalone HTML receipt beside it.
...
Repair accepts only a failed or blocked `static`, `test`, `review`, or `artifact` receipt produced by the current runtime for this repository. A failed review receipt must contain an intact, conflict-free repair manifest bound to the same repository, base/head identities, fingerprints, patch digest, and findings. The correction prompt includes that manifest and requires every action to be independently verified and implemented in one coherent delta; it may not stop after the first item or defer known work. Automatic review repair is limited to one pass: the resulting repair branch is independently re-reviewed, but another repair branch is not generated from it. A receipt transported between clean CI workspaces is rebound only after its repository identity, configuration digest, and before/final fingerprints match the target. Repair passes a structured prompt containing the untrusted receipt—not the raw receipt as instructions—to a temporary autonomous Git sandbox with no inherited Git remotes, repository hooks, or Git credentials. Its clean process environment exposes only configured `repair`-scope secrets. Before running verification or applying anything, Sam Harness blocks if a secret exposed to the correction appears in new regular-file bytes, a changed symlink target, or an added patch line; an unchanged baseline occurrence is not treated as a new leak. It also blocks escaping symlinks, Git-control changes, stale target state, and budget overruns. A successful delta must pass fresh `static` and `test` checks before being applied to the unchanged target. Independent review of the resulting change is still required. The receipt records the correction-only patch and `repair_patch_sha256`.
...
```bash
sam-harness upgrade /path/to/repository --to 0.9.2 --answers /tmp/sam-harness-v0.9-answers.json
```
...
- At runtime, `review` and `repair` require network authority. Staging, the configured migration command, and observation require network plus deploy authority; production and rollback require network plus both deploy and release authority.
...
- Ordinary GitHub `pull_request`/`merge_group` jobs and GitLab merge-request jobs receive no bound agent secrets. For a bound GitHub scope, the default-branch-owned `.github/workflows/sam-harness-agents.yml` uses `pull_request_target`, a `repository_dispatch` named `sam_harness_merge_group_review`, or a failed-run `workflow_run`. It never listens directly to `merge_group`: an external App/webhook must forward the exact provider merge-queue head SHA, current default-branch base SHA, and `gh-readonly-queue` ref as data. The resolver re-fetches those provider refs before review and before check conclusion; missing dispatch or drift blocks the required check. It runs the released CLI version declared by the trusted configuration with trusted base configuration, scopes model secrets to the matching review or repair step, and never runs target setup, hooks, caches, local actions, or repository commands. Automatic repair is never published for a merge-group run. GitHub mixed review/repair bindings move only the bound scope and retain applicable credential-free work locally.
- GitLab does not generate an agent loop inside the target repository. When `mode: external`, the configured external project owns all agent review, correction, and publishing regardless of binding mix and must publish the configured required status for the exact current MR head. The MR pipeline omits every local agent review, correction, and publisher job; missing external status blocks merge. Set `gitlab_external_pipeline_policy: true` only when a protected GitLab Pipeline Execution Policy also owns static, test, and artifact gates; generated local phase jobs then remain active on trusted branches but are disabled for merge requests.
- Creating provider credentials and configuring the control plane remain external administration tasks. Restrict the GitHub agent environment to default/protected branches, require human reviewers, enable prevent-self-review, keep the dedicated App ID and private key only in that environment, require its exact status check, and remotely read back all settings. Configure and read back the GitLab external project, protected variables/environment, status check, protected branch, and approval rules. A fork, unavailable secret, missing check, missing trusted base configuration, missing released runtime, or changed head fails closed; never silently skip or downgrade the gate. The generated Sam Harness self workflow therefore needs that matching release and base configuration established through an approved trusted bootstrap path before agents receive secrets. Use a provider `ci_secret_waivers` reason only when approved commands genuinely need no credentials.
...
- The six reviewers are `architecture`, `security`, `correctness`, `test_quality`, `business_rules`, and `simplicity`. Their commands must be independently configured and attested as filesystem-read-only. Each returns `review_complete: true` and every actionable finding with an exact `required_change` and observable `acceptance`. The pre-merge prompt carries the base/head fingerprints plus `review_patch_path` and `review_patch_sha256`; the receipt binds the canonical patch and one hashed repair manifest containing all in-scope findings. Malformed or incomplete reviewer JSON, an in-scope P0/P1 finding, an out-of-scope P0/P1, repository mutation, or changed base/head blocks review. Out-of-scope P2/P3 suggestions are excluded from correction while remaining visible in the HTML receipt.
