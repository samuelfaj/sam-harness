...
```bash
sam-harness upgrade /path/to/repository --to 0.7.2 --answers /tmp/sam-harness-v0.7-answers.json
```
...
- GitLab does not generate an agent loop inside the target repository. When `mode: external`, the configured external project owns all agent review, correction, and publishing regardless of binding mix and must publish the configured required status for the exact current MR head. The MR pipeline omits every local agent review, correction, and publisher job; missing external status blocks merge.
