...
Trusted review uses the pinned released harness, trusted base configuration, explicit `--review-base`, `--review-base-sha`, and `--review-head-sha`; its receipt binds the exact provider SHAs, fingerprints, patch, and SHA-256. Missing trusted control-plane inputs or credentials fails closed.
